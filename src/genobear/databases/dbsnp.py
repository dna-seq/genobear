"""
dbSNP database download and processing functions.
"""
...
from genobear.config import (
    DEFAULT_DBSNP_URL, DEFAULT_ASSEMBLIES_MAP, DEFAULT_URL_PREFIX,
...
async def download_dbsnp(
    ...
    with start_action(action_type="download_dbsnp_files", 
        ...
        if convert_to_parquet_files:
            conversion_results = await convert_dbsnp_to_parquet(
                assemblies, releases, output_folder, force, parquet_batch_size
            )
        ...
        for assembly in assemblies:
            ...
            create_default_symlink(releases[0], assembly_folder)
...
async def convert_dbsnp_to_parquet(
    assemblies: List[str],
    releases: List[str],
    output_folder: Optional[Path] = None,
    force: bool = False,
    ...
    with start_action(action_type="convert_dbsnp_to_parquet") as action:
        results = {}
